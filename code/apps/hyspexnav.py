...
class HyspexNavApplication:
    ...
    def _generate_output_files(self, controlfinder: ControlFinder):
        ...
        generate_btn = controlfinder.find_by_auto_id(
            control_type="Button",
            auto_id="HySpexNav.splitter.layoutWidgetFlightLines.btnGenerateOutput")
        generate_btn.invoke()

    def _clear_flightlines(self, controlfinder: ControlFinder):
        """Click the 'Clear Flight Lines' button to clear 
        the staged flight lines for the next iteration."""
        clear_btn = controlfinder.find_by_auto_id(
            control_type="Button",
            auto_id="HySpexNav.splitter.layoutWidgetFlightLines.btnFlightLineClear")
        clear_btn.invoke()
    ...
    def run(self):
        ...
        with ApplicationManager(app_path=self.app_path,
            ...
            for input_folder, output_folder in zip(self.input_folders, self.output_folders):
                ...
                settings_path = get_base_path(
                    __file__).parent / "configs" / "conf_hyspex_nav.ini"
                ...
                # Wait a bit to ensure analysis is done before generating output
                time.sleep(1)
                self._generate_output_files(main_window_cf)
                wait_for_file_count(
                    folder=output_folder, expected_count=6, pattern="*.txt", timeout_s=10)
