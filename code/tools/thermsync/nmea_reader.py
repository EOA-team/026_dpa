...
class NmeaReader:
    ...
    def get_timestamp(zda: ZDA) -> str:
        """Return 'YYMMDD_HHMMSSms' from a ZDA sentence."""
        yy = str(zda.year)[-2:]
        mm = f"{zda.month:02d}"
        dd = f"{zda.day:02d}"
        hh = zda.time_utc[0:2]
        ...
        return f"{yy}{mm}{dd}_{hh}{mi}{ss}{ms}"

    def save_sentence_to_csv(self, sentence, filename: str, path: str | Path) -> None:
        if sentence is None:
            return
        file_path = Path(path) / f"{filename}.csv"
        row = vars(sentence)
        write_header = not file_path.exists()
        with open(file_path, "a", newline="") as f:  # ← file_path, not path
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        timestamp = reader.get_timestamp(reader.get_latest(ZDA))
